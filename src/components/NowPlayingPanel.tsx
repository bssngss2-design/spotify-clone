...
export function NowPlayingPanel({ onClose, onToggleQueue }: NowPlayingPanelProps) {
  ...
  return (
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-1 w-52 bg-[#282828] rounded-md shadow-2xl py-1 z-[70]">
                  ...
                  <button onClick={() => { if (currentSong?.artist) router.push(`/artist/${encodeURIComponent(currentSong.artist)}`); setMenuOpen(false); }} className="w-full text-left px-3 py-2.5 text-sm text-[#eaeaea] hover:text-white hover:bg-[#3e3e3e] flex items-center gap-3 transition-colors">
                    <svg className="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12,11A5,5,0,1,0,7,6,5.006,5.006,0,0,0,12,11Zm0-8A3,3,0,1,1,9,6,3,3,0,0,1,12,3ZM3,22a9,9,0,0,1,18,0,1,1,0,0,1-2,0A7,7,0,0,0,5,22a1,1,0,0,1-2,0Z" /></svg>
                    Go to artist
                  </button>
                  ...
                  <div className="border-t border-[#3e3e3e] my-1" />
                  <button onClick={() => { setCreditsOpen(true); setMenuOpen(false); }} className="w-full text-left px-3 py-2.5 text-sm text-[#eaeaea] hover:text-white hover:bg-[#3e3e3e] flex items-center gap-3 transition-colors">
                  ...
                  <div className="relative" onMouseEnter={() => setShareSubOpen(true)} onMouseLeave={() => setShareSubOpen(false)}>
                    <button onClick={() => setShareSubOpen(!shareSubOpen)} className="w-full text-left px-3 py-2.5 text-sm text-[#eaeaea] hover:text-white hover:bg-[#3e3e3e] flex items-center justify-between transition-colors">
                    ...
                    {shareSubOpen && (
                      <div className="absolute left-full top-0 ml-1 w-[200px] bg-[#282828] rounded-md shadow-2xl py-1 z-[80]">
                        <button onClick={() => { navigator.clipboard.writeText(window.location.href); toast("Link copied to clipboard"); setMenuOpen(false); setShareSubOpen(false); }}
                        ...
                        <button onClick={() => { toast("Embed is not available"); setMenuOpen(false); setShareSubOpen(false); }}
                          className="w-full text-left px-3 py-2.5 text-sm text-[#eaeaea] hover:bg-[#3e3e3e] hover:text-white flex items-center gap-3 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-4 pb-4">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-[#b3b3b3] font-semibold hover:underline cursor-pointer" onClick={() => setCreditsOpen(true)}>Show all</span>
            </div>
            {currentSong.artist && (
    ...
    {creditsOpen && currentSong && <CreditsModal song={currentSong} onClose={() => setCreditsOpen(false)} />}
  </>
  );
}
